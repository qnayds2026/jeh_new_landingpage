import { CheckCircle2, ArrowRight, Lock, Clock3, ShieldCheck, BadgeIndianRupee } from "lucide-react";
...
export default function HeroCard({onEnrollClick,}) {
  return (
    <div className="sticky top-8 overflow-hidden rounded-2xl border border-[#EDEAE3]/10 bg-[#12100D]">
      ...
      <div className="relative bg-[#FFB627] py-3 text-center">
        <p className="rotate-[-0.5deg] text-sm font-bold uppercase tracking-widest text-[#0A0908]">
          Limited Time Offer
        </p>
        {/* Perforated edge, like a torn ticket */}
        <div
          className="absolute -bottom-[5px] left-0 h-[10px] w-full bg-[#12100D]"
          style={{
            maskImage:
      ...
      <div className="p-8 pt-10">
        ...
        <div className="my-8 rounded-xl border border-dashed border-[#EDEAE3]/20 p-6">
          ...
          <div className="flex items-baseline justify-between">
            ...
            <span className="flex items-center gap-1 font-serif text-5xl font-medium text-[#FFB627]">
        ...
        <div className="space-y-3">
          {features.map((item) => (
            <div key={item} className="flex items-center gap-3">
              <CheckCircle2 size={18} className="shrink-0 text-[#FFB627]" />
              <span className="text-[#EDEAE3]">{item}</span>
            </div>
          ))}
        </div>
        ...
        {/* Trust */}
        <div className="mt-8 space-y-4 border-t border-[#EDEAE3]/10 pt-6 text-[#A8A29B]">
